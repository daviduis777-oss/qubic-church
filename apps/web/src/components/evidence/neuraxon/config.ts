...
// ============================================================
// Research Scene Config (Qortex v2)
// ============================================================

export const RESEARCH_SCENE_CONFIG = {
  LAYERS: {
    terrain:   { label: 'Matrix Terrain',    defaultVisible: true,  yOffset: 0 },
    anomalies: { label: 'Anomaly Markers',   defaultVisible: true,  yOffset: 0 },
    bridges:   { label: 'Bridge Network',    defaultVisible: true,  yOffset: 15 },
    spiral:    { label: 'Eigenvalue Spiral',  defaultVisible: true,  yOffset: 30 },
    cycle:     { label: 'Period-4 Cycle',    defaultVisible: true,  yOffset: 38 },
    addresses: { label: 'Bitcoin Addresses', defaultVisible: true,  yOffset: 0 },
    messages:  { label: 'Encoded Messages',  defaultVisible: true,  yOffset: 0 },
  },
  CAMERA_PRESETS: {
    overview: { position: [0, 30, 55] as const, target: [0, 15, 0] as const, label: 'Overview' },
    matrix:   { position: [0, 15, 25] as const,  target: [0, 0, 0] as const,  label: 'Matrix' },
    bridges:  { position: [0, 25, 30] as const, target: [0, 15, 0] as const, label: 'Bridges' },
    spiral:   { position: [15, 38, 18] as const, target: [0, 30, 0] as const, label: 'Spiral' },
    cycle:    { position: [0, 45, 18] as const, target: [0, 38, 0] as const, label: 'Cycle' },
  },
  COLORS: {
    terrainNeg: '#3B82F6',
    terrainNeutral: '#6B7280',
    terrainPos: '#D4AF37',
    anomalyCol22: '#6bcb77',
    anomalyCol97: '#4d96ff',
    anomalySpecial: '#ffd93d',
    bridgeSymmetry: '#60A5FA',
    bridgeXor: '#D4AF37',
    eigenDominant: '#3B82F6',
    coopPhase: '#22C55E',
    restPhase: '#F59E0B',
  },
} as const;

export type ResearchCameraPreset = keyof typeof RESEARCH_SCENE_CONFIG.CAMERA_PRESETS;
