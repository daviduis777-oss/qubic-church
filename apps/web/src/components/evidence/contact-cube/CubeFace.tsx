...
import { CUBE_FACE_SIZE, COLOR_THEMES, DEPTH_SCALE, CELL_SIZE, CUBE_SCALE, ANOMALY_COLUMN_COLORS } from './constants'
  ...
  useEffect(() => {
    ...
    for (let row = 0; row < CUBE_FACE_SIZE; row++) {
      ...
      for (let col = 0; col < CUBE_FACE_SIZE; col++) {
        ...
        if (anomaly) {
          // Highlight anomalies — color-coded by column pair
          if (anomaly.special) {
            ...
            // Color by column pair (columns that sum to 127)
            const colColor = ANOMALY_COLUMN_COLORS[anomaly.pos[1]]
            color = new THREE.Color(colColor || '#FF6B6B')
          }
        } else {
    ...
    if (attr instanceof THREE.InstancedBufferAttribute) {
      (attr.array as Float32Array).set(colorArray)
      attr.needsUpdate = true
    } else {
      mesh.geometry.setAttribute('color', new THREE.InstancedBufferAttribute(colorArray, 3))
    }

    // Update material only if not already vertex-colored
...
// Outline for cube face edges with emissive glow for bloom
