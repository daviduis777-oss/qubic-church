...
function calculateDifference(value1: number, value2: number): {
  ...
  // Two's Complement identity: symmetric cells sum to -1, not 0
  const isSymmetric = value1 + value2 === -1
...
function getMatchColor(matchType: 'exact' | 'symmetric' | 'anomaly', theme: ColorTheme): THREE.Color {
  switch (matchType) {
    ...
    case 'symmetric':
      return new THREE.Color('#3B82F6') // Blue - symmetric (a + b = -1)
    case 'anomaly':
...
function OverlayLegend({ position }: { position: [number, number, number] }) {
  return (
    <group position={position}>
      ...
      <Text position={[0.4, 1, 0]} fontSize={0.18} color="#22C55E" anchorX="left" anchorY="middle">
        Exact Match
      </Text>
      ...
      <Text position={[0.4, 0, 0]} fontSize={0.18} color="#3B82F6" anchorX="left" anchorY="middle">
        Symmetric (-1)
      </Text>
      ...
      <Text position={[0.4, -1, 0]} fontSize={0.18} color="#EF4444" anchorX="left" anchorY="middle">
        Anomaly
      </Text>
    </group>
  )
