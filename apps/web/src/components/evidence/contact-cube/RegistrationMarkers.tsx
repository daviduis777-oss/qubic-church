...
// Floating text label using drei Text
...
// Genesis position marker for [72,72] = -27
function GenesisPositionMarker() {
  const meshRef = useRef<THREE.Mesh>(null)
  const ringRef = useRef<THREE.Mesh>(null)

  const position = useMemo(() =>
    matrixToWorldPosition(SPECIAL_POSITIONS.genesis.row, SPECIAL_POSITIONS.genesis.col),
    []
  )
  ...
  useFrame(({ clock }) => {
    ...
    // Rotating ring (opposite direction to primer)
    ringRef.current.rotation.z = -time * 0.4
    ringRef.current.rotation.y = Math.sin(time * 0.25) * 0.3
  ...
  return (
    <group position={position}>
      {/* Central sphere — cyan/blue theme */}
      <mesh ref={meshRef}>
...
export function RegistrationMarkers({
  ...
  return (
    <group>
      ...
      {/* Genesis marker for [72,72] */}
      <GenesisPositionMarker />
