...
export function useContactCubeData(): UseContactCubeDataReturn {
  ...
  const loadData = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const [matrixResponse, anomalyResponse] = await Promise.all([
        fetchWithTimeout('/data/anna-matrix-min.json', FETCH_TIMEOUT, signal),
      ...
      if (err instanceof Error && err.name === 'AbortError') {
        // Don't set error on component unmount abort
        return
      }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      symmetryPercentage: anomalyData.metadata?.symmetryPercentage ?? 99.58,
    }
