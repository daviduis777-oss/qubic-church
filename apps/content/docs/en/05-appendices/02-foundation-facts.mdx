...
<Callout type="warning" title="Reading scope: facts vs hypotheses">
This appendix mixes **measured facts** (matrix dimensions, point-symmetry rate, Genesis-block timestamps, on-chain transaction data) with **hypothesized derivations** (address generation paths like `K12(K12(seed)) + step + XOR`). Sections explicitly marked "Hypothesized", "Possible", or "Hypothesis" are exactly that — strong patterns, not proven derivation procedures. Where this matters most: §4 (Address Family) generation methods are hypotheses that match the 0x7b family prefix, but no script reproduces 1CFB deterministically from a known seed. See `/docs/03-results/28-grok-path-verification` for the most recent walk-back on the related "Master Key" / Satoshi-Troll subclaims (tier-3, unverifiable).
</Callout>
...
- **Matches found**: 0
- **95% confidence upper bound on hit rate under uniform-random null**: ≈ 6 × 10⁻⁷ per attempt (Wilson score interval for 0 successes in 4.94 M trials)
- **Conclusion**: No deterministic derivation path was discovered within the tested corpus. Under the uniform-random null this is compatible with early-access being impossible via the tested methods, but does not formally exclude untested derivation schemes.
