...
**Properly assessed:**
- The Patoshi pattern (distinct nonce-range signature separating Satoshi-mined blocks from other early miners) is a robust, independently reproduced result (Lerner 2013, "The Well-Deserved Fortune of Satoshi Nakamoto"). The precise p-value depends on the specific nonce-range statistic tested; as published, the pattern is highly significant against a uniform-nonce null, though the exact exponent varies by analyst and test.
- The 1CFB vanity address is unusual (1 in 195,112 for a random 4-character prefix match) but not extraordinary — roughly 1,500 such prefix matches are expected across early Bitcoin given its address volume.
- The connection between CFB and Satoshi has not been subjected to any rigorous independent test and remains a hypothesis.
