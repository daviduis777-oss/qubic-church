...
Under a naive uniform-random null (each property assumed independent), the joint probability of four blocks sharing these three properties (hex "DEAD" presence, divisibility by 27, unspent status) is estimated at roughly p &lt; 10⁻⁸. This figure is **not** Bonferroni-corrected for the space of 4-hex pattern words (there are 16⁴ ≈ 65 k possible 4-hex combinations that could have been searched) or for the divisor pool, so the effective multiple-testing burden is substantial. The pattern is real and on-chain, but the bare p-value should be interpreted as suggestive rather than conclusive.
...
| 137 | Prime; equals ⌊1/α⌋ (integer landmark, suggestive coincidence not derivation) | Formula offset parameter |
