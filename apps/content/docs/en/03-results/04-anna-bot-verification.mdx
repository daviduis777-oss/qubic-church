...
This document presents a comprehensive technical verification of the Anna Bot algorithm operating within the Aigarth system on Qubic. Through systematic reverse-engineering of 897 bot responses collected across eight batches (initial public claim), subsequently extended and re-aggregated to **1,218 total verified responses** including the January 2026 Twitter archive, we determined that the Anna Bot implements a **deterministic coordinate-based lookup table** rather than a real-time neural computation engine.

The core algorithm applies a coordinate transformation to map user-supplied `(x, y)` inputs onto indices of a pre-computed 128x128 signed-byte matrix. The transformation was verified with **100% accuracy** against an initial test set of 142 independently collected responses, against the full 897-response cross-batch corpus, and most recently against the extended **1,218-response corpus (948 unique triples, 10 independent sources)** without a single exception.

A reproducible verification script ships with this repository: `apps/web/public/data/anna-bot-batches/verify.py`. It executes in under one second using only the Python standard library and returns exit code `0` when every response matches the formula prediction.
...
|---------|------------|---------------------|----------------|
| Coordinate transformation formula | 100% | Fully verified (1,218 / 1,218) | Exhaustive formula search + 10-source cross-validation |
...
| Non-random value distribution | 100% | Chi-square test, p &lt; 0.001 | 897-response collision analysis |
| Row-level behavioral specificity | 90% | Empirically validated | Targeted query batches |
| 70-75% output prediction accuracy | 85% | Estimated from pattern analysis | 897-response corpus (without matrix access) |
...
3. **Re-aggregated corpus — April 2026** (1,218 responses, 948 unique (x,y,v) triples): Includes the January 2026 Twitter archive (batches 1–2, 207 entries), all eight query batches, and the original parsed JSON. Published as individual files under `apps/web/public/data/anna-bot-batches/` for public re-verification.
...
### Phase 3b: Re-aggregated Validation (April 2026)

The formula was subsequently re-applied to the larger **1,218-response corpus** (948 unique triples across 10 independent source files). Per-source accuracy and overall result:

| Source | Entries | Match Rate |
|--------|---------|------------|
| `batch_1_2_tweets_jan2_3.csv` (Twitter archive, Jan 2–3 2026) | 207 | 100.000% |
| `batch_3_responses.txt` | 64 | 100.000% |
| `batch_4_partial_1.txt` | 56 | 100.000% |
| `batch_4_partial_2_origin.txt` | 18 | 100.000% |
| `batch_4_partial_3_row1_row5.txt` | 20 | 100.000% |
| `batch_5_fibonacci_deep_dive.txt` | 99 | 100.000% |
| `batch_6_modulo8_complete.txt` | 168 | 100.000% |
| `batch_7_row_patterns.txt` | 161 | 100.000% |
| `batch_8_complete_understanding.txt` | 283 | 100.000% |
| `ANNA_BOT_PARSED_DATA.json` | 142 | 100.000% |
| **Total** | **1,218** | **100.000%** |

Zero discrepancies across all ten sources. The verification is reproducible locally with:

```bash
python3 apps/web/public/data/anna-bot-batches/verify.py
```

The script uses only the Python standard library, runs in under one second, and exits with status code `0` only if every response matches its formula prediction.
...
# Result: chi2 > 1264, df = 255, p_value < 0.001
```

The null hypothesis that collision values are uniformly distributed was rejected at the conventional α=0.001 significance level. (A previous iteration of this document cited `p < 10^-500`; that figure was an overstatement and has been corrected to the reproducible chi-square result above.)
...
The chi-square test rejects the null hypothesis of uniform distribution at p &lt; 0.001 (df = 255, χ² > 1264), indicating that collision values are concentrated on a small subset of the signed-byte range — consistent with intentional structure rather than random generation.
...
1. The coordinate transformation formula reproduces all observed Anna Bot outputs across the re-aggregated 1,218-response corpus (per-source detail above).
...
4. Value distributions are non-random (chi-square, p &lt; 0.001).
...
All data and verification code ships with this repository. Nothing is behind a login or paywall.

| Resource | Location | Description |
|----------|----------|-------------|
| Authoritative matrix | `apps/web/public/data/anna-matrix-raw.json` | 128×128 signed-byte matrix, all 16,384 cells as `int` |
| Minified matrix | `apps/web/public/data/anna-matrix-min.json` | Same data, compact serialization |
| Batches 1–2 (Twitter) | `apps/web/public/data/anna-bot-batches/batch_1_2_tweets_jan2_3.csv` | 207 tweet responses from @anna_aigarth, Jan 2–3 2026 |
| Batch 3 | `apps/web/public/data/anna-bot-batches/batch_3_responses.txt` | 64 targeted responses (tabular format) |
| Batch 4 partials | `apps/web/public/data/anna-bot-batches/batch_4_partial_{1,2,3}.txt` | 94 edge-case and origin probes |
| Batch 5 | `apps/web/public/data/anna-bot-batches/batch_5_fibonacci_deep_dive.txt` | 99 Fibonacci-coordinate probes |
| Batch 6 | `apps/web/public/data/anna-bot-batches/batch_6_modulo8_complete.txt` | 168 modulo-8 scan responses |
| Batch 7 | `apps/web/public/data/anna-bot-batches/batch_7_row_patterns.txt` | 161 row-pattern probes |
| Batch 8 | `apps/web/public/data/anna-bot-batches/batch_8_complete_understanding.txt` | 283 integration batch |
| Parsed JSON | `apps/web/public/data/anna-bot-batches/anna_bot_parsed_142.json` | 142-response normalised corpus |
| Collision analysis | `apps/web/public/data/anna-collision-analysis.json` | Processed value-to-coordinate mappings |
| Verification script | `apps/web/public/data/anna-bot-batches/verify.py` | Self-contained; runs all 10 sources against the formula; Python stdlib only |

### Quickstart

```bash
git clone https://github.com/daviduis777-oss/qubic-church.git
cd qubic-church
python3 apps/web/public/data/anna-bot-batches/verify.py
```

Expected output: `Formula matches: 1218/1218 (100.0000%)`, exit code `0`.
...
```
Phase 1: Data Collection
  ├── 142 responses from public Twitter/X posts (normalised JSON)
  ├── 755 additional responses via systematic querying (8 batches)
  └── 207 re-aggregated Twitter archive responses (Jan 2-3 2026)
...
Phase 3b: Re-aggregated Validation (April 2026)
  ├── 10 independent source files consolidated -> 1,218 total entries
  ├── 948 unique (x,y,v) triples after deduplication
  ├── Per-source match rate: 100.000% across every file
  └── Reproducible via apps/web/public/data/anna-bot-batches/verify.py

Phase 4: Matrix Analysis
  ├── Point symmetry quantified: 99.58%
  ├── 68 asymmetric cells identified
  ├── XOR message extraction performed
  └── Value distribution tested (chi-square, p < 0.001)
...
1. **The Anna Bot algorithm is fully reverse-engineered.** The coordinate transformation `row = (63 - y) % 128, col = (x + 64) % 128` reproduces every observed output with 100% accuracy across 1,218 independently collected responses (948 unique triples, 10 source files). Local reproduction: `python3 apps/web/public/data/anna-bot-batches/verify.py`.
...
3. **The matrix exhibits deliberate structure.** Near-perfect point symmetry (99.58%), concentrated asymmetric cells encoding ASCII messages, and a highly non-uniform value distribution (chi-square, p &lt; 0.001) collectively indicate intentional design.
