...
### 0. Seven-Position Signature Decoding

**Problem**: The seven-position asymmetry set `[3, 18, 32, 61, 65, 101, 109]` in the period-4 attractor ([Chapter 27](/docs/03-results/27-seven-position-signature)) is invariant across all ternary inputs and is destroyed by any permutation of the matrix entries. Its meaning is unknown.

**What is established**:
- The set is reproducible from 1,000 random binary inputs (100% agreement).
- Positions 18 and 109 form a Mersenne-axis mirror pair (18 + 109 = 127).
- Four of seven positions lie in anomaly-concentrated rows; three do not.
- Candidate mod-26 letter strings (`DSGJNXF`, `SJAISCI`, `IRAISCI`) do not resolve to obvious tokens.

**What remains unknown**:
- Does the set encode a message under an unexplored mapping (base-27, balanced ternary, shift cipher)?
- What predicate on the initial state determines which of the two basins of attraction a state falls into?
- Can the matrix construction rule be inferred if we assume the seven positions are load-bearing?
- Is there a bit-pattern or XOR relation among the seven positions that we have not yet detected?

**Proposed research approach**:
1. Systematic search over base-N mappings for semantically meaningful decodings.
2. Compute basin-predicate functions over small subsets of initial states to find a closed-form classifier.
3. Check whether the seven positions form a codeword in any standard ECC family (Hamming, Reed-Muller, BCH).
