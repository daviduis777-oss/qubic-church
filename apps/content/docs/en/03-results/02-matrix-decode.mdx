...
| Dominant eigenvalue phase ≈ pi/2 (0.4565° = 0.507% deviation) | **Proven** | 99% | 1.2 |
...
| Deviation from pi/2 | 0.0080 rad (0.4565°, 0.507%) |

The dominant eigenvalue is almost purely imaginary. Its phase angle of 90.46 degrees
is within 0.51% of exactly 90 degrees (pi/2) — a deviation of 0.4565°. This is the mathematical origin of the
period-4 cycle: four applications of a 90-degree rotation return to the starting point
(4 x 90 = 360 degrees). (Earlier docs stated 0.16% — that figure was incorrect by a factor of ~3; the correct value of 0.4565° / 0.51% matches `30-period-4-anti-attractor.mdx` and direct numpy verification of `np.linalg.eigvals(anna).max(key=abs)`.)
...
8. Simulate ternary dynamics:
   ...
   ```
9. Verify all 1000 random inputs converge to one of exactly two period-4 cycles,
   with antipodal symmetry breaking at the fixed set `[3, 18, 32, 61, 65, 101, 109]`.
   See [Chapter 27](/docs/03-results/27-seven-position-signature) for the full
   asymmetry-signature and null-test analysis.
