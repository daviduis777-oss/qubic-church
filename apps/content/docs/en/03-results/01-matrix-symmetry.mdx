...
- **Expected symmetric pairs by chance**: approximately 64 out of 8,192 pairs (each pair has 1/256 probability of summing to −1 under a uniform-random signed-byte null)
- **Observed symmetric pairs**: 8,158 out of 8,192
- **Standard deviations from expectation**: > 100 sigma
- **Chi-square test**: null hypothesis of uniform-random byte assignment rejected at p &lt; 0.001

A prior iteration of this chapter cited `p < 10^-500`; that figure was not independently reproduced and has been retracted. The observed deviation is already visible at &gt;100σ without requiring an exponent that large to be meaningful — the symmetry is clearly not a statistical artifact, but an intentional structural property of the matrix. See `/docs/03-results/12-statistical-validation` for the full retraction record.
...
## 4.5 Dynamical Consequence: Two-Cycle Attractor

The cell-level mirror symmetry has a direct dynamical consequence. When any sign-ternary 128-vector `s` is iterated through the matrix under `s := sign(M @ s)`, the system converges to a period-4 cycle with antipodal structure `A = -C`, `B = -D`. Across 1,000 random binary initial states, every trajectory terminates in one of exactly **two distinct cycles**, and the antipodal symmetry breaks at an identical set of seven positions:

```
[3, 18, 32, 61, 65, 101, 109]
```

This seven-position signature is invariant across all tested inputs and is destroyed by any random permutation of the matrix entries. See [Chapter 27](/docs/03-results/27-seven-position-signature) for the full analysis.
