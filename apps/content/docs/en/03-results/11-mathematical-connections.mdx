...
Expected frequency of primes near magnitude 100: approximately 15%. Among the four "key structural constants" listed above, all four are prime. This set of constants was identified after inspecting the system, so it is a post-hoc selection — the 100% figure describes the curated list, not a blind sample. A pre-registered selection rule would be required to rule out selection bias.
...
Expected frequency of perfect squares among integers: approximately 3%. All six of the architectural constants listed above are perfect squares. As with the prime enrichment above, the list was curated after inspecting the system — the 100% figure describes the selected set, not a blind draw from the space of all Qubic-related integers.

**Classification**: **[VERIFIED]** — the primality / square-status of each listed constant is arithmetically verifiable. **[HYPOTHESIS]** — whether this reflects deliberate architectural selection (rather than selection bias in which constants we consider "key") remains open and would require pre-registered replication to settle.
